(function () {
  ...
  async function triggerAction(action) {
    addLog('DEBUG: triggerAction called for ' + action, 'info');
    if (!bulkForm) {
      addLog('DEBUG: bulkForm not found!', 'error');
      return;
    }
    ...
    // Inject selected VMs as hidden inputs into the form because checkboxes are outside
    // First remove old ones
    bulkForm.querySelectorAll('input[name="vms"]').forEach(el => el.remove());
